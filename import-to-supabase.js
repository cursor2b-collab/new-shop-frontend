...
async function importData() {
  ...
  for (let i = 0; i < countries.length; i += batchSize) {
    ...
    if (error) {
      console.error(`❌ 导入国家数据失败 (批次 ${Math.floor(i / batchSize) + 1}):`, error.message);
      // 如果批量失败，尝试逐条导入
      for (const country of batch) {
        const { error: singleError } = await supabase
          .from('jm273_countries')
          .upsert(country, { onConflict: 'id' });
        if (!singleError) {
          importedCountries++;
        }
      }
    } else {
      importedCountries += batch.length;
      console.log(`✅ 已导入 ${importedCountries}/${countries.length} 个国家`);
    }
  }

  console.log(`\n✅ 国家数据导入完成！共 ${importedCountries} 条记录\n`);
